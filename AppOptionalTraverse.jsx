...
export default function MerchantTreeView({ merchants }) {
  ...
  function getAllChild(childNode = null, collectedNodes = []) {
  ...
  function getOnChange(checked, nodes) {
    ...
    if (traverse) {
      ...
      let array = checked
        ? [...selected, ...[nodes.id]]
        : selected.filter((v, i) => v !== nodes.id);
      setSelected(array);
    }
  }
  ...
  const renderTree = (nodes) => {
    ...
    if (traverse) {
      const allSelectedChildren = parentMap[nodes.id]?.every((childNodeId) =>
  ...
  return (
    <>
      ...
      <button onClick={() => setTraverse(!traverse)}>Toggle Traversing</button>
    </>
  );
}
